 ...
 * @param {string} config.operator - The operator to use when comparing the
 * result to the target.
 * @param {number} config.modifier - A numeric modifier to apply to the roll.
 * @param {string} config.target - The value to compare the rolled result
 * against.
 * @param {number} config.rollType - Does this formula roll twice? If so, does
 * it take the higher or lower Roll?
